...
## v1.1 맞춤법 · 온보딩
- **맞춤법 검사**(기본 켜짐): macOS 시스템 사전, 밑줄 + 컨텍스트 메뉴 제안 (네트워크 없음)
- **자동 수정**(기본 꺼짐): 설정 또는 편집 메뉴에서 토글
- **확장자 예외**: `log`, `json`, `swift` 등 코드/로그 확장자에서는 검사 생략 (설정에서 목록 편집)
- **Welcome** 첫 실행 안내, **What's New** 버전 업그레이드 시 (도움말 메뉴에서 다시 열기)
- 글꼴 설정 설명 보강, 상태 표시줄에서 줄/열 클릭 → 줄로 이동, 인코딩 메뉴 구역 분리
